...
An atomic vector is just a simple vector of data. In fact, you've already made an atomic vector, your `die` object from [Project 1: Weighted Dice](#sec-project-dice). You can make an atomic vector by grouping some values of data together with `c`:
...
A double vector stores regular numbers. The numbers can be positive or negative, large or small, and have digits to the right of the decimal place or not. In general, R will save any number that you type in R as a double. So, for example, the die you made in [Project 1: Weighted Dice](#sec-project-dice) was a double object:
...
or a 1 × 2 × 3 cube (which has 1 row, 2 columns, and 3 "slices"). This is a three-dimensional structure, but R will need to show it slice by slice on your two-dimensional computer screen:
...
You can also use `class` to set an object's `class` attribute, but this is usually a bad idea. R will expect objects of a class to share certain traits, such as attributes, that your object may not possess. You'll learn how to make and use your own classes in [Project 3: Slot Machine](#sec-project-slots).
...
R can open many types of files---not just CSVs. Visit [Loading and Saving Data in R](#sec-appendix-dataio) to learn how to open other common types of files in R.
